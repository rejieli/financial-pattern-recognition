...
RealTimeMultiThreadChart::RealTimeMultiThreadChart(QObject *parent) : QObject(parent)
{ 
    //
...
RealTimeMultiThreadChart::~RealTimeMultiThreadChart()
{
    delete currentChart;
}
...
//
void RealTimeMultiThreadChart::onDataTimer()
{
    // The current time
...
//
void RealTimeMultiThreadChart::onChartUpdateTimer(QmlChartViewer *viewer)
{
    if (m_currentIndex > 0)
    {
        //
...
// Draw chart
//
void RealTimeMultiThreadChart::drawChart(QmlChartViewer *viewer)
{
    ...
    viewer->setChart(currentChart = c);
}

//
// Draw track cursor when mouse is moving over plotarea
//
void RealTimeMultiThreadChart::drawTrackCursor(QmlChartViewer *viewer, int mouseX)
{
    double trackLinePos = trackLineLabel((XYChart *)viewer->getChart(), mouseX);
    trackLineIsAtEnd = (m_currentIndex <= 0) || (trackLinePos == trackLineEndPos);
    viewer->updateDisplay();
}

void RealTimeMultiThreadChart::drawFullChart(QmlViewPortControl *vpc)
{
    XYChart *c = new XYChart(700, 100, 0xffffee);
    c->setPlotArea(0, 0, c->getWidth(), c->getHeight(), 0x99ccff);
    vpc->setChart(c);
}
...
//
double RealTimeMultiThreadChart::trackLineLabel(XYChart *c, int mouseX)
{
